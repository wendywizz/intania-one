...
import { TYPE_MEETING_TODAY } from '@/constants/type-meeting';
...
function MeetingListItem({ meeting }: MeetingListItemProps) {
  ...
  return (
    <ThemedView style={styles.itemCard} lightColor="#FFFFFF" darkColor="#151718">
      ...
      {schedule ? <ThemedText style={styles.itemMeta}>{schedule}</ThemedText> : null}
      {place ? <ThemedText style={styles.itemMeta}>{place}</ThemedText> : null}
      {detail ? <ThemedText style={styles.itemDetail}>{detail}</ThemedText> : null}
    </ThemedView>
  );
}
  ...
  const renderContent = () => {
    ...
    return (
      <FlatList
        ...
        refreshControl={
          <RefreshControl refreshing={isRefreshing} onRefresh={() => loadMeetings(true)} />
        }
        renderItem={({ item }) => <MeetingListItem meeting={item} />}
        ListEmptyComponent={
          <ThemedView style={styles.emptyCard} lightColor="#FFFFFF" darkColor="#151718">
            <ThemedText style={styles.emptyMessage}>{TEXT.MEETING_NO_MEETINGS_TODAY}</ThemedText>
          </ThemedView>
        }
      />
    );
