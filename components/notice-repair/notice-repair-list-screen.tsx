...
export function NoticeRepairListScreen({ title, description, listType, staffId, segments }: Props) {
  ...
  const openDetail = (job: NoticeRepairJob) => {
    navPush({
      ...
      params: { repair_id: job.repair_id, staff_id: staffId, role: currentRole },
    } as Parameters<typeof navPush>[0]);
  };

  return (
    <ThemedView style={styles.container}>
      <NavTopBar title={TEXT.PUBLIC_REPAIR_TITLE} backHref="/" rightContent={roleSwitcher} />

      <View style={styles.panelHeader}>
        <ThemedText type="subtitle">{title}</ThemedText>
        <ThemedText style={styles.panelDescription}>
          {description ?? TEXT.PR_LIST_SUBTITLE}
        </ThemedText>
      </View>
