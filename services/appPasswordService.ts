...
/**
 * The app's own unlock passcode — deliberately separate from the device
...
/** Digits in an app passcode — fixed length, like the iOS Settings passcode. */
export const PASSCODE_LENGTH = 6;

const isWellFormed = (passcode: string) =>
  passcode.length === PASSCODE_LENGTH && /^\d+$/.test(passcode);
...
export async function setAppPassword(password: string) {
  if (!isSupported || !isWellFormed(password)) return false;
