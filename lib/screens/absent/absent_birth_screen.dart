...
class AbsentBirthScreen extends StatefulWidget {
  final String? id;

  const AbsentBirthScreen({
    super.key,
    this.id,
  });

  @override
  State<AbsentBirthScreen> createState() => _AbsentBirthScreenState();
}

class _AbsentBirthScreenState extends State<AbsentBirthScreen>
        ...
        if (user != null) {
          ...
          if (widget.id == null) {
            result = await AbsentService()
                .initAbsentData(staffId, TYPE_ABSENT_BIRTH);
          } else {
            result = await AbsentService()
                .getData(widget.id.toString(), TYPE_ABSENT_BIRTH);
          }
    ...
    if (_formKey.currentState!.validate()) {
      ...
      if (avParts!.length == 2) {
        ...
        var data = {
          ...
          'absence_type': TYPE_ABSENT_BIRTH,
        ...
        Timer(CONNECTION_TIMEOUT, () async {
          ...
          if (widget.id != null) {
            ...
            result = await AbsentService()
                .updateData(widget.id.toString(), data, TYPE_ABSENT_BIRTH);
          } else {
            result = await AbsentService().addData(data, TYPE_ABSENT_BIRTH);
          }
